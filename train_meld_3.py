...
class MaskedNLLLoss(nn.Module):
    ...
    def forward(self, pred, target, mask=None):
        ...
        if mask is None:
            ...
            return loss
            # mask = torch.ones_like(target).unsqueeze(0)
            # if type(self.weight) == type(None):
            #     loss = self.loss(pred , target) / target.size(0)
            # else:
            #     loss = self.loss(pred, target) \
...
def generate_pos(input_ids):
    # positions = []
    # for pos, id in enumerate(input_ids):
    #     if pos == 0:
    #         break
    #     else:
...
def train_or_eval_model(model, loss_Func, dataloader, epoch, optimizer=None, scheduler = None, model_type='albert',
    ...
    for data in tqdm(dataloader):
        ...
        # num_chunck = len(cpt_graph_i)
        # for idx in range(num_chunck):
        #
        #     for cpt in cpt_graph_i[idx]:
        #         cpt = cpt.cuda() if torch.is_tensor(cpt) and cuda_ else cpt
        # for chunk in cpt_graph_i:
        #     for item in chunk:
        #         item = item.cuda() if torch.is_tensor(item) and cuda_ else item
        ...
        # elif model_type == 'roberta' or model_type == 'roberta_large':
        inputs = {'input_ids': sent_ids, 'attention_mask': mask, 'token_type_ids': token_types}
