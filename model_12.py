...
class Model(nn.Module):
    ...
    def forward(self, inputs, str_src, str_dst, str_edge_type, chunks, label, loss_func, train=True, eps=1e-8):
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if masks.sum()==0:
                ...
                if self.rel_fun in ['vector', 'ones', 'linear', 'nora']:
                    if self.rel_fun == 'nora':
                        rels = torch.zeros_like(rels).long()
            ...
            if self.num_feature == 4:
                ...
                feat_ = out_[utt_idx].unsqueeze(1).unsqueeze(1)
                feat = self.CoAtt(feat_).squeeze(1).squeeze(1)
                if self.use_layer_norm:
                    output = torch.log_softmax(self.linear(self.ac_tanh(self.layer_norm(self.dropout(self.fusion(feat))))), dim=1)
                else:
    ...
    def symbolic_proc(self, relatt_out_chunk, dst_emb, weights, sentics,src_masks, masks, chunk_size, num_src, num_dst):
        ...
        # todo: alpha = (src_masks.ne(0).unsqueeze(2) * torch.softmax(omega, dim=-1)).unsqueeze(2).repeat(1, 1, self.input_dim, 1).transpose(2, 3)
