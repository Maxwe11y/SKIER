...
from dataloader_3 import MELDDataset, DailyDialogueDataset, get_chunk
...
def train_or_eval_model(model, loss_Func, dataloader, epoch, optimizer=None, scheduler = None, model_type='albert',
    ...
    for data in tqdm(dataloader):
        ...
        log_prob, loss, log_prob_aux = model.forward(inputs, str_src, str_dst, str_edge_type, cpt_graph_i, labels_,
        ...
        pred_aux_ = torch.argmax(log_prob_aux, 1)
        preds_aux.append(pred_aux_.data.cpu().numpy())
    ...
    avg_accuracy_aux = round(accuracy_score(labels, preds_aux, sample_weight=masks) * 100, 2)
    avg_fscore_aux = round(f1_score(labels, preds_aux, sample_weight=masks, average='weighted') * 100, 2)

    return avg_loss, avg_accuracy, labels, preds, masks, avg_fscore, class_report, avg_accuracy_aux, avg_fscore_aux
...
if __name__ == '__main__':
    ...
    if Configs.fine_tune:
        checkpoint = torch.load(Configs.model_path + 'model_5_3.pth')
    ...
    for e in range(Configs.epochs):
        ...
        train_loss, train_acc, _, _, _, train_fscore, _, _, _ = train_or_eval_model(model, loss_Func=loss_function,
        ...
        with torch.no_grad():
            test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, test_class_report, test_acc_aux, test_fscore_aux = \
        ...
        if best_fscore == None or best_fscore < test_fscore:
            ...
            # torch.save(state, Configs.model_path + 'model_5_3.pth')

        print('epoch {} train_loss {} train_acc {} train_fscore {} test_loss {} test_acc {} test_fscore {} test_acc_aux {} test_fscore_aux {} time {}'. \
                format(e + 1, train_loss, train_acc, train_fscore, test_loss,
                       test_acc, test_fscore, test_acc_aux, test_fscore_aux, round(time.time() - start_time, 2)))
