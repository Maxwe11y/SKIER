    ...
    def forward(self, inputs, str_src, str_dst, str_edge_type, chunks, label, loss_func, train=True):
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if self.num_feature == 4:
                feat_1 = torch.cat((out_[utt_idx], hidden_rgcn[utt_idx],
                                  relatt_out[utt_idx]), dim=-1)
                feat_2 = symbolic_repr

                output_1 = torch.log_softmax(self.linear(self.ac_tanh(self.dropout(self.fusion(feat_1)))), dim=1)
                output_2 = torch.log_softmax(self.linear_2(feat_2), dim=1)

            elif self.num_feature == 3:
                feat_1 = torch.cat((out_[utt_idx], hidden_rgcn[utt_idx],
                                  symbolic_repr), dim=-1)
                feat_2 = relatt_out[utt_idx]
                output_1 = torch.log_softmax(self.linear(self.ac_tanh(self.dropout(self.fusion(feat_1)))), dim=1)
                output_2 = torch.log_softmax(self.linear_2(feat_2), dim=1)

            elif self.num_feature == 2:
                feat_1 = torch.cat((out_[utt_idx], relatt_out[utt_idx],
                                    symbolic_repr), dim=-1)
                feat_2 = hidden_rgcn[utt_idx]
                output_1 = torch.log_softmax(self.linear(self.ac_tanh(self.dropout(self.fusion(feat_1)))), dim=1)
                output_2 = torch.log_softmax(self.linear_2(feat_2), dim=1)
            elif self.num_feature == 1:
                feat_1 = torch.cat((hidden_rgcn[utt_idx], relatt_out[utt_idx],
                                    symbolic_repr), dim=-1)
                feat_2 = out_[utt_idx]
                output_1 = torch.log_softmax(self.linear(self.ac_tanh(self.dropout(self.fusion(feat_1)))), dim=1)
                output_2 = torch.log_softmax(self.linear_2(feat_2), dim=1)
            ...
            loss_1 = loss_func(output_1, label[utt_idx]) / len_dial
            loss_2 = loss_func(output_2, label[utt_idx]) / len_dial
            loss = loss_1 + loss_2
            ...
            output_.append(output_1.data)
            output_aux.append(output_2.data)
            losses += loss_1.item()
            del symbolic_repr
            if masks.sum() != 0:
        ...
        return output_, losses, output_aux
