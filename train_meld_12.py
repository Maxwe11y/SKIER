...
def train_or_eval_model(model, loss_Func, dataloader, epoch, optimizer=None, scheduler = None, model_type='albert',
    ...
    for data in tqdm(dataloader):
        ...
        case_pred = list(np.int32(pred_.data.cpu().numpy()))
        case_label = list(np.int32(labels_.data.cpu().numpy()))
        preds_case.append({'id': data[-1][0], 'pred': case_pred, 'label': case_label})
    ...
    if data_type in ['meld', 'emory']:
        ...
        if data_type == 'meld':
            return avg_loss, avg_accuracy, labels, preds, preds_case, masks, avg_fscore, class_report
        else:
...
if __name__ == '__main__':
    ...
    if Configs.data_type == 'meld':
        best_va_fscore, best_fscore, best_loss, best_label, best_pred, best_pred_case, best_mask = None, None, None, None, None, None, None
        for e in range(Configs.epochs):
            start_time = time.time()
            print('base_lr {} lr {}'.format(optimizer.param_groups[0]['lr'], optimizer.param_groups[1]['lr']))
            train_loss, train_acc, _, _,_, _, train_fscore, _, = train_or_eval_model(model, loss_Func=loss_function,
            ...
            with torch.no_grad():
                valid_loss, valid_acc, _, _,_, _, valid_fscore, _, = train_or_eval_model(model, loss_Func=loss_function,
            ...
            with torch.no_grad():
                test_loss, test_acc, test_label, test_pred, test_pred_case, test_mask, test_fscore, test_class_report = \
            ...
            if best_va_fscore == None or best_va_fscore < valid_fscore:
                if train_fscore>=valid_fscore:
                    best_va_fscore, best_fscore, best_loss, best_label, best_pred, best_mask, best_pred_case = \
                        valid_fscore, test_fscore, test_loss, test_label, test_pred, test_mask, test_pred_case
                    # state = {'net': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': e}
                    # torch.save(state, Configs.model_path + 'model_case.pth')
        ...
        # with open('./case_study.pkl', 'wb') as f:
        #     pickle.dump(best_pred_case, f)
        ...
        for e in range(Configs.epochs):
            ...
            print('epoch {} train_loss {} train_acc {} train_fmacro {} train_fmicro {} valid_loss {} valid_acc {} valid_fmacro {} valid_fmicro {} test_loss {} test_acc {} test_fmacro {} test_fmicro {} time {}'. \
        ...
        print('Test performance..')
        print('Fmacro {} Fmicro {} accuracy {}'.format(best_fmacro, best_fmicro, round(accuracy_score(best_label, best_pred, sample_weight=best_mask) * 100,
                                                   2)))
